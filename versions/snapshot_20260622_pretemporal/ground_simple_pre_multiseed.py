...
def main():
    ...
    for t, plist in sorted(by_t.items()):
        ...
        for p in plist:
            ...
            if mid is not None:                                # (2) ORANGE thumb-index point
                for m, s in _seg_at(mid[0], mid[1]):
            ...
            _dbg(f"[POOL] {tag} t={t:.1f} {hand} anchor=({int(ax)},{int(ay)}) shared={shared}: "
                 + " | ".join(f"{c[3]}{(':' + c[0][:10]) if c[0] else ''} manip={c[4]:.2f} d={c[5]:.2f} sam={c[2]:.2f}"
                              for c in pool))
            seed_viz = ([(mid[0], mid[1], (255, 140, 0))] if mid else [])   # orange = thumb-index point (red pinch dropped)
