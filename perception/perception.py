...
def extract(video, fps_target=30.0, conf=0.3):
    ...
    chain = {"L": [], "R": []}                            # normalized [thumb line kp1-4, index line kp5-8] per
    #                                                    frame — the two FINGER LINES; a small precision-held
    #                                                    object sits in the pinch region BETWEEN them
    while True:
        ok, bgr = cap.read()
        if not ok:
            break
        if fi % step == 0:
            ...
            for side, cl in (("L", 0), ("R", 1)):
                ...
                if h is None:
                    ...
                    chain[side].append([[[np.nan, np.nan]] * 4, [[np.nan, np.nan]] * 4])
                else:
                    ...
                    tip[side].append([[float(kp[i][0]) / W, float(kp[i][1]) / H]
                                      ...
                                      for i in (4, 8)])
                    # thumb finger line (kp1-4) + index finger line (kp5-8) — the two FINGER LINES; the
                    # pinch region BETWEEN them is where a small precision-held object sits.
                    chain[side].append([
                        [[float(kp[i][0]) / W, float(kp[i][1]) / H] if (kp is not None and kp[i][2] > 0.3)
                         else [np.nan, np.nan] for i in (1, 2, 3, 4)],
                        [[float(kp[i][0]) / W, float(kp[i][1]) / H] if (kp is not None and kp[i][2] > 0.3)
                         else [np.nan, np.nan] for i in (5, 6, 7, 8)]])
        fi += 1
    cap.release()
    return {"t": np.array(T), "fps": fps, "W": W, "H": H,
            ...
            "chain": {k: np.array(v, dtype=float) for k, v in chain.items()},  # [thumb kp1-4, index kp5-8]
